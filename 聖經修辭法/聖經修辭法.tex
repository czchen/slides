...
\begin{frame}{主要平行體}
  \begin{itemize}
    \item\textbf{同義 (Synonymous) 平行}：第二句使用不同詞彙重述或強化第一句的核心概念。\\
      (\bibleref{Psalms 19:1}) 諸天述說神的榮耀；穹蒼傳揚他的手段。
    \item\textbf{對照 (Antithetic) 平行}：第二句與第一句形成鮮明對比，透過反襯突顯真理與智慧。\\
      (\bibleref{Proverbs 10:1}) ... 智慧之子使父親歡樂；愚昧之子叫母親擔憂。
    \item\textbf{綜合 (Synthetic) 平行}：第二句不單純重複，而是補充、延伸或進一步發展第一句的思想。\\
...
\begin{frame}{交叉對稱 (Chiasm)}
  ...
  交叉對稱的四個結構層級：
  \begin{itemize}
    \item\textbf{微觀交叉對稱}：單句/短語層面
    \item\textbf{段落交叉對稱}：獨立敘事/單篇詩歌層面
    \item\textbf{宏觀/書卷級交叉對稱}：跨章節/整卷書層面
    \item\textbf{嵌套交叉對稱}：交叉結構內部嵌入子交叉
  \end{itemize}
...
\begin{frame}{替代與代表 (Substitutions)}
  ...
  \begin{itemize}
    \item \textbf{換喻/轉喻 (Metonymy)}：以密切相關的事物（如作者代作品、容器代內容物）替代。\\
      (\bibleref{Luke 16:29}) 亞伯拉罕說：他們有摩西和先知的話可以聽從。
    \item \textbf{提喻 (Synecdoche)}：以部分代表整體，或以整體代表部分。\\
      (\bibleref{Romans 3:20}) 所以凡有血氣的，沒有一個因行律法，能在神面前稱義。
      \begin{itemize}
        \item \textbf{與兩極表達法 (Merism) 的差異}：提喻是用「單一局部」代表整體（如「血氣」代人類）；兩極表達法是用「兩個極端」涵蓋全部（如「天地」代宇宙）。
      \end{itemize}
  \end{itemize}
\end{frame}
